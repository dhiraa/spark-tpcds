...
trait DataGenerator extends Serializable {
  def generate(sparkContext: SparkContext,
                name: String,
                partitions: Int,
...
abstract class Tables(sqlContext: SQLContext, scaleFactor: String,
  ...
  case class Table(name: String, partitionColumns: Seq[String], fields: StructField*) {
    ...
    def genData(location: String,
      ...
      val writer = if (partitionColumns.nonEmpty) {
        ...
        if (clusterByPartitionColumns && maxRecordPerFile > 0) {
          ...
          if (maxRecordPerFile > 0 && numRows > maxRecordPerFile) {
            ...
            data.coalesce(numPartitions).write
          }
        } else {
          println(s"No partitioning applied, whole data is written with default partition : $data.rdd.getNumPartitions")
          log.info(s"No partitioning applied, whole data is written with default partition: $data.rdd.getNumPartitions")
          data.write
        }
      }
  ...
  def genData(location: String,
              ...
              clusterByPartitionColumns: Boolean,
              filterOutNullPartitionValues: Boolean,
              tableFilter: Set[String] = Set.empty[String],
    ...
    if (!tableFilter.isEmpty) {
      tablesToBeGenerated = tablesToBeGenerated.filter(t => tableFilter.contains(t.name))
